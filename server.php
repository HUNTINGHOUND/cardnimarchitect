<?php
	...
	if($argc < 3 || !is_numeric($argv[1]) || !is_numeric($argv[2]) || !is_numeric($argv[3])) {
		echo("[ERROR] Please provide port number, number of stones, and number of cards\n");
		echo("[ERROR] Example command: php server.php 5000 30 25\n");
		exit(-1);
	}
	...
	send_message($connections[1], "1 $num_stones $card_max\n", $is_websocket[1]);
	send_message($connections[2], "2 $num_stones $card_max\n", $is_websocket[2]);
